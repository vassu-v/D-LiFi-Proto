...
inline void irSendRaw(String header, String message = ""){
  ...
  // 🔴 Red LED ON when starting transmission
  LED_TX_ON();
  ...
  // 🔴 Red LED OFF after transmission complete
  LED_TX_OFF();
...
inline bool irReceive(String &header, String &message){
  ...
  if(irReceiveString(line)){
    ...
    // 🟢 Green LED blink when receiving packet
    LED_RX_ON();
...
/*
 * LiFi Broadcast (Node to Phones) - Start Transmission
 * Starts 10-second visual broadcast (non-blocking)
 * 
 * DEMO MODE: Triggers 10-second blink pattern
 * In production: Replace with actual LiFi modulation driver
 * 
 * NOTE: This just starts the broadcast. The actual blinking
 * is handled non-blocking in the main loop via lifiProcessBlink()
 */
inline void lifiTransmit(String message){
  Serial.println();
  Serial.println("╔════════════════════════════════════╗");
  Serial.println("║   LIFI BROADCAST TO PHONES         ║");
  Serial.println("╚════════════════════════════════════╝");
  Serial.print("Message: ");
  Serial.println(message);
  Serial.println(">>> LiFi: Starting 10-second visual broadcast...");
  Serial.println("════════════════════════════════════");
  Serial.println();
}

/*
 * LiFi Blink Processor (Non-blocking)
 * Call this in main loop to handle lamp blinking
 * 
 * Uses external state variables:
 * - lifiBlinkActive (bool): Is blink in progress?
 * - lifiBlinkStartTime (unsigned long): When blink started
 * - lifiBlinkState (bool): Current LED state
 * - lifiLastToggleTime (unsigned long): Last toggle time
 */
void lifiProcessBlink(bool &active,
  ...
  // Blink forever, ignore active/startTime
  if(now - lastToggle >= 500){
    lastToggle = now;
    state = !state;
    digitalWrite(LAMP_LIGHT_PIN, state ? HIGH : LOW);
  }
}
 ...
 * 
 * FIXED: Deduplication strategy:
 * - Discard INIT only if: (same initID) AND (receivedHop >= myHop - 1)
 * - This allows better paths to update the gradient while preventing storms
 */
inline void processInit(String header){
  ...
  Serial.print("Current myHop: "); Serial.println(myHop);
  
  // ✅ FIXED: Smart deduplication based on initID and hop comparison
  // Discard if: same initID AND receivedHop is not better than current path
  if(initID == lastInitID && receivedHop >= myHop - 1){
    #if DEBUG_GRADIENT
      Serial.println(">>> GRADIENT: INIT discarded");
      Serial.print("    Reason: Same initID (");
      Serial.print(initID);
      Serial.print(") and hop not better (received=");
      Serial.print(receivedHop);
      Serial.print(" >= myHop-1=");
      Serial.print(myHop - 1);
      Serial.println(")");
      Serial.println(">>> GRADIENT: Prevents INIT storm while allowing better paths");
    #endif
    Serial.println("════════════════════════════════════");
    Serial.println();
  ...
  // Process INIT: either new ID or better hop
  bool shouldForward = false;
  
  if(initID == lastInitID){
    // Same ID with better hop (receivedHop < myHop - 1)
    uint8_t oldHop = myHop;
    myHop = receivedHop + 1;
    shouldForward = true;
    
    #if DEBUG_GRADIENT
      Serial.println(">>> GRADIENT: BETTER PATH found!");
      Serial.print("    myHop updated: ");
      Serial.print(oldHop);
      Serial.print(" → ");
  ...
  // Forward INIT with incremented hop
  if(shouldForward){
    ...
    Serial.print(">>> GRADIENT: Forwarding INIT with hop=");
...
inline void generateSOS(){
  ...
  irSend(header);  // Send header-only to all 4 directions (LED_TX will blink)
 ...
 * 
 * Parameters for LiFi blink state (passed by reference from main loop):
 * - lifiBlinkActive: Set to true to start 10-second blink
 * - lifiBlinkStartTime: Set to millis() when starting blink
 */
inline void forwardPacket(String header, String message, 
                          String &latestLiFiMessage, 
                          unsigned long &lastLiFiBroadcastTime,
                          bool &lifiBlinkActive,
                          unsigned long &lifiBlinkStartTime){
  if(header.length() < 9){
  ...
  if(type == MSG_TYPE_INIT && header.length() == HEADER_LENGTH_INIT){
    processInit(header);  // Now includes deduplication
    return;
  }
  ...
  if(type == MSG_TYPE_SOS && header.length() == HEADER_LENGTH_SOS){
    ...
    if(myHop <= msgHop + GRADIENT_TOLERANCE){
      ...
      if(isNew(src, 0)){
        ...
        irSend(newHeader);  // LED_TX will blink automatically
      }
    } else {
      #if DEBUG_GRADIENT
  ...
  if(type == MSG_TYPE_MESSAGE && header.length() == HEADER_LENGTH_MESSAGE){
    ...
    if(myHop <= msgHop + GRADIENT_TOLERANCE){
      ...
      if(isNew(src, receivedHash)){
        ...
        irSend(newHeader, message);  // LED_TX will blink automatically
      }
    } else {
      #if DEBUG_GRADIENT
  ...
  if(header.length() == HEADER_LENGTH_STANDARD){
    ...
    if(isNew(src, receivedHash)){
      irSend(header, message);  // LED_TX will blink automatically
    }
    ...
    if(type == MSG_TYPE_BROADCAST && dst == BROADCAST_ID && IS_FROM_HQ(src)){
      ...
      lifiTransmit(message);  // Log the broadcast
      
      // Start 10-second blink for visual LiFi transmission
