...
inline void irSendRaw(String header, String message = ""){
  ...
  for(int i = 0; i < 4; i++){
    IrSender.begin(txPins[i], ENABLE_LED_FEEDBACK); // Re-init for this pin
    delay(10);
    irSendString(packet.c_str());

    if(i < 3){
      delay(IR_DIRECTION_GAP);
    }
  }
  IrSender.begin(IR_TX_FRONT, ENABLE_LED_FEEDBACK); // Reset to default
}
 ...
 * Uses irReceiveString from ir.h to get a complete packet
 * Parses packet into header and message
 */
inline bool irReceive(String &header, String &message){
  String packet;
  if(irReceiveString(packet)){
    int separator = packet.indexOf('|');

    if(separator != -1){
      // Packet has header and message
      header = packet.substring(0, separator);
      message = packet.substring(separator + 1);
    } else {
