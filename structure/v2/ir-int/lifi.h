 ...
 * Sends header (and optional message) via IR using IRremote library
 */
inline void irSendRaw(String header, String message = ""){
  ...
  // Send header
  String headerWithNewline = header + "\n";
  irSendString(headerWithNewline.c_str());
  
  // Send message if present
  if(message.length() > 0){
    delay(50);  // Small gap between header and message
    String messageWithNewline = message + "\n";
    irSendString(messageWithNewline.c_str());
  }
}
